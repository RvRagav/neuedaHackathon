...
#     def generate_transactions(self, start_date, end_date, transaction_count=50, custom_categories=None):
...
#                 particulars = self._generate_credit_particulars(category)
#                 debit = 0
#                 credit = amount
#             else:
#                 amount = round(random.uniform(10, 500), 2)
#                 particulars = self._generate_debit_particulars(category)
#                 debit = amount
#                 credit = 0
...
#             return f"UPI/OLA/RIDE/{self.fake.msisdn()[-10:]}"
#         elif category == 'Entertainment':
#             return f"UPI/NETFLIX/SUBSCRIPTION/{self.fake.msisdn()[-10:]}"
#         elif category == 'Shopping':
#             return f"UPI/AMAZON/ORDER#{self.fake.bothify(text='A#####')}"
#         elif category == 'Utilities':
#             return f"UPI/PHONEPE/RECHARGE/{self.fake.msisdn()[-10:]}"
#         elif category == 'Transfer':
#             return f"IMPS/{self.fake.first_name().upper()}/IMPS{random.randint(100000, 999999)}"
#         elif category == 'Cash':
#             return f"CASH WDL/ATM/BRANCH{random.randint(100, 999)}"
#         else:
#             return f"POS/{self.fake.company().upper()}/BILL#{self.fake.bothify(text='P####')}"

#     def _generate_credit_particulars(self, category):
#         if category == 'Salary':
#             return f"SALARY/{self.fake.company().upper()}/ACXXXX{random.randint(1000,9999)}"
#         elif category == 'Transfer':
#             return f"NEFT/CR/{self.fake.company().upper()}/{self.fake.first_name().upper()}/{random.randint(10000000, 99999999)}"
#         elif category == 'Investment':
#             return f"UPI/CRED/CREDITCARD/{self.fake.msisdn()[-10:]}"
#         elif category == 'Cash':
#             return f"CASH DEPOSIT/ATM/BRANCH{random.randint(100, 999)}"
#         else:
#             return f"UPI/{self.fake.company().upper()}/TXN#{random.randint(10000,99999)}"
...
#         summary_info = [
#             ["Opening Balance:", f"{self.account_details['currency']} {opening_balance:,.2f}"],
#             ["Total Debits:", f"{self.account_details['currency']} {total_debits:,.2f}"],
#             ["Total Credits:", f"{self.account_details['currency']} {total_credits:,.2f}"],
#             ["Closing Balance:", f"{self.account_details['currency']} {closing_balance:,.2f}"]
#         ]
...
#         transaction_data = [['Date', 'Description', 'Debit', 'Credit', 'Balance']]
#         for _, row in self.transaction_df.iterrows():
#             transaction_data.append([
#                 row['Date'].strftime('%d-%b-%Y'),
#                 row['Particulars'],
#                 f"{self.account_details['currency']} {row['Debit']:,.2f}" if row['Debit'] > 0 else "",
#                 f"{self.account_details['currency']} {row['Credit']:,.2f}" if row['Credit'] > 0 else "",
#                 f"{self.account_details['currency']} {row['Balance']:,.2f}"
#             ])

#         transaction_table = Table(transaction_data, colWidths=[0.8 * inch, 3 * inch, 1 * inch, 1 * inch, 1 * inch], repeatRows=1)
...
#             pdf_buffer = generator.generate_pdf_statement()
...
#     def generate_transactions(self, start_date, end_date, transaction_count=50, custom_categories=None, include_category=True):
...
#         show_decimals = st.checkbox("Show Decimal Values", value=True)
